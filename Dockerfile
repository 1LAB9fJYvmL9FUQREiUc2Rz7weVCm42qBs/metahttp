...
COPY src/identity.xslt /usr/lib/
COPY src/metahttp-compiler.xslt /usr/lib/
COPY src/xslt.sh /usr/local/bin/
COPY src/metahttp-server.sh /usr/local/bin/
COPY src/exec-wrapper.sh /usr/local/bin/
COPY src/startup-servers.sh /usr/local/bin/
#ENTRYPOINT /usr/bin/socat TCP4-LISTEN:50774,fork,reuseaddr "EXEC:'exec-wrapper.sh metahttp-server.sh',nofork,stderr"
